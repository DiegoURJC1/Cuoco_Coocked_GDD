    ...
    useEffect(() => {
        ...
        const scanAll = async () => {
            ...
            for (const file of fileList) {
                ...
                } catch { newTitles[file] = file.replace(/\.[^/.]+$/, ""); }
            }
    ...
    return (
        <div className="player-container">
            <div className="visual-wrapper">
                ...
                <div className={`cd-svg-container ${isPlaying ? 'is-playing' : ''}`}>
                    <svg width="200" height="200" viewBox="0 0 200 200">
                        ...
                        <path d="M 150 100 A 50 50 0 0 1 100 150" fill="none" stroke="white" strokeWidth="2" opacity="0.15" strokeLinecap="round" />
                        <path d="M 50 100 A 50 50 0 0 1 100 50" fill="none" stroke="white" strokeWidth="2" opacity="0.15" strokeLinecap="round" />
            ...
            <div className="controls">
                <button className="control-btn" onClick={() => setCurrentIndex(p => (p - 1 + fileList.length) % fileList.length)}>
                    <svg width="32" height="32" fill="currentColor" viewBox="0 0 16 16"><path d="M4 4a.5.5 0 0 1 1 0v3.248l6.267-3.636c.52-.302 1.192.105 1.192.702v7.372c0 .597-.672 1.004-1.192.702L5 8.752V12a.5.5 0 0 1-1 0V4z"/></svg>
                </button>
                <button className="play-pause-btn" onClick={togglePlay}>
                    {isPlaying ? (
                        <svg width="30" height="30" fill="currentColor" viewBox="0 0 16 16"><path d="M5.5 3.5A1.5 1.5 0 0 1 7 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5zm5 0A1.5 1.5 0 0 1 12 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5z"/></svg>
                    ) : (
                ...
                <button className="control-btn" onClick={() => setCurrentIndex(p => (p + 1) % fileList.length)}>
                    <svg width="32" height="32" fill="currentColor" viewBox="0 0 16 16"><path d="M12.5 4a.5.5 0 0 0-1 0v3.248L5.233 3.612C4.713 3.31 4 3.717 4 4.314v7.372c0 .597.713 1.004 1.233.702L11.5 8.752V12a.5.5 0 0 0 1 0V4z"/></svg>
                </button>
            </div>

            <div className="playlist-container">
                {fileList.map((file, idx) => (
                    <div key={idx} className={`playlist-item ${idx === currentIndex ? 'active' : ''}`} onClick={() => { setCurrentIndex(idx); setIsPlaying(true); }}>
